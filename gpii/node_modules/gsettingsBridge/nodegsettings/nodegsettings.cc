...
NAN_METHOD(get_gsetting_keys) {
  ...
  Local<Array> togo = New<Array>(size);
  for (i = 0; keys[i]; i++) {
...
NAN_METHOD(get_gsetting) {
  ...
  if (g_variant_type_equal(type,G_VARIANT_TYPE_DOUBLE)) {
    info.GetReturnValue().Set(New(g_variant_get_double(variant)));
  }
  else if (g_variant_type_equal(type,G_VARIANT_TYPE_INT32)) {
    info.GetReturnValue().Set(New(g_variant_get_int32(variant)));
  }
  else if (g_variant_type_equal(type,G_VARIANT_TYPE_STRING)) {
    info.GetReturnValue().Set(New(g_variant_get_string(variant,NULL)).ToLocalChecked());
  }
  else if (g_variant_type_equal(type,G_VARIANT_TYPE_BOOLEAN)) {
    info.GetReturnValue().Set(New<Boolean>(g_variant_get_boolean(variant)));
  }
  else {
    g_print("The type is %s\n", g_variant_type_peek_string(type));
    ThrowError(New("Need to implement reading that value type").ToLocalChecked());
...
NAN_METHOD(set_gsetting) {
  ...
  settings = g_settings_new(*String::Utf8Value(schema));
  if (info[2]->IsBoolean()) {
    status = g_settings_set_boolean(settings, *String::Utf8Value(key),
                                    info[2]->BooleanValue());
  }
    ...
    type = g_variant_get_type(variant);
    if (g_variant_type_equal(type,G_VARIANT_TYPE_DOUBLE)) {
      status = g_settings_set_double(settings, *String::Utf8Value(key),
                                     info[2]->ToNumber()->Value());
    }
    else if (g_variant_type_equal(type,G_VARIANT_TYPE_INT32)) {
      status = g_settings_set_int(settings, *String::Utf8Value(key),
                                  info[2]->ToInt32()->Value());
    }
    else {
      g_print("The type is %s\n", g_variant_type_peek_string(type));
      ThrowError(New("We haven't implemented this number type yet!").ToLocalChecked());
    }
    g_variant_unref(variant);
  }
    ...
    variant = g_settings_get_value(settings, *String::Utf8Value(key));
    type = g_variant_get_type(variant);
    status = g_settings_set_string(settings, *String::Utf8Value(key),
                                   *String::Utf8Value(val));
    g_variant_unref(variant);
  }
  else {
    ThrowError(New("We haven't implemented this type yet!").ToLocalChecked());
  }
  g_settings_sync();
  g_clear_object(&settings);

  info.GetReturnValue().Set(New<Boolean>(status));
}
