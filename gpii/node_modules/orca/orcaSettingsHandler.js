...
(function () {
    ...
    function applySettings(app) {
        ...
        var currentSettingsInProfile = userSettings.profiles[userSettings.general.activeProfile[PROFILE_ID]];

        var applier = fluid.makeHolderChangeApplier(currentSettingsInProfile, {
        ...
        fluid.each(settings, function (settingVal, settingKey) {
            var oldValue = fluid.get(currentSettingsInProfile, settingKey,
            ...
            fluid.set(currentSettingsInProfile, settingKey, settings[settingKey],
                      fluid.model.escapedSetConfig);
        });

        userSettings.profiles[userSettings.general.activeProfile[PROFILE_ID]] = currentSettingsInProfile;
    ...
    gpii.orca.getImpl = function (payload) {
        ...
        newSettingsResponse = fluid.transform(settings, function (settingVal, settingKey) {
            var value = fluid.get(currentSettingsInProfile, settingKey,
                fluid.model.escapedGetConfig);
            return value;
        });
    ...
    gpii.orca.setImpl = function (profile) {
    ...
    gpii.orca.get = function (payload) {
        return gpii.settingsHandlers.invokeSettingsHandler(gpii.orca.getImpl, payload);
    };

    gpii.orca.set = function (payload) {
        return gpii.settingsHandlers.invokeSettingsHandler(gpii.orca.setImpl, payload);
    };
