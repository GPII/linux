...
(function() {
    ...
    fluid.defaults ("gpii.processReporter", {
      ...
      invokers: {
        ...
        getProcesses: {
          funcName: "gpii.processReporter.getProcessList",
          args: []
        },
        findProcessByPid: {
    ...
    gpii.processReporter.findSolutionsByCommands = function (commandNames) {
      if (!!commandNames) {
       return fluid.accumulate (commandNames, function (aCommand, matches) {
          var procInfos = 
            gpii.processReporter.findProcessesByCommand (aCommand);
    ...
    gpii.processReporter.hasSwitchedRunState = function (monitor) {
      ...
      var isRunning =
        gpii.processReporter.isRunning (monitor.newProcInfo.state);
    ...
    gpii.processReporter.updateProcInfo = function (procInfo) {
      ...
      if (procInfo.state === "NoSuchProcess") {
        newProcInfo =
          gpii.processReporter.findFirstProcessByCommand (procInfo.command);
      }
      else {
      ...
      if (newProcInfo === null) {
        newProcInfo =
          gpii.processReporter.initProcInfoNotRunning (procInfo.command);
      }
      return newProcInfo;
    };
