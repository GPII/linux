...
var processReporter = gpii.processReporter();
...
var runStateChangeHandler = function (procInfo) {
  console.log (
    ...
    " (" +
    ( processReporter.isRunning (procInfo.state) ? "Running" : "Stopped" ) +
    ")"
  );
};
...
var orcaProcInfo = processReporter.initProcInfoNotRunning ("orca");
var orcaProcInfos = processReporter.findProcessesByCommand ("orca");
if (orcaProcInfos.length > 0) {
  orcaProcInfo = orcaProcInfos[0];
}

// Start the periodic check of the change in run-state for orca.
// To see any changes, startstop orca using, say, the accessibility menu, or
// the command line:
// gsettings set org.gnome.desktop.a11y.applications screen-reader-enabled true/false
var states = {};
states.trackRunState =
  processReporter.trackRunState (orcaProcInfo, runStateChangeHandler);
states.trackState =
  processReporter.trackState (orcaProcInfo, stateChangeHandler);
...
seekInput.on ("line", function (answer) {
  ...
  switch (answer.trim()) {
    case "yes":
      ...
      processReporter.stopTrackingRunState (
        runStateChangeHandler, states.trackRunState
      );
      processReporter.stopTrackingState (stateChangeHandler, states.trackState);
      seekInput.close();
      break;
