  ...
  var path = require ("path"),
    ...
    jqUnit.test (
      ...
      function() {
        ...
        jqUnit.assertTrue (
          "Check nodejs is running", processesBridge.isRunning (procInfo.state)
        );
        procInfo = processesBridge.initProcInfoNotRunning ("grep");
        jqUnit.assertFalse (
          "Check nonexistent process running", processesBridge.isRunning (procInfo)
        );
    });
    ...
    jqUnit.test (
      ...
      function() {
        ...
        jqUnit.assertFalse (
          "Check null process", processesBridge.hasStateChanged (catMonitor)
        );
    ...
    jqUnit.test (
      ...
      function() {
        ...
        jqUnit.assertFalse (
          "Check null process", processesBridge.hasSwitchedRunState (grepProcMonitor)
        );
        var grep = spawn ("grep", ["ssh"]);
    ...
    jqUnit.test (
      ...
      function() {
        var orcaProcInfo = processesBridge.findFirstProcessByCommand ("orca");
        ...
        procTests.wait5sec();
        orcaProcInfo = processesBridge.findFirstProcessByCommand ("orca");
        ...
        if (orcaProcInfo !== null) {
          var orcaProcNewState = processesBridge.findProcessByPid (orcaProcInfo.pid);
          jqUnit.assertNull ("Stop Orca", orcaProcNewState);
          var orcaMonitor = processesBridge.initMonitor (orcaProcInfo);
          processesBridge.hasStateChanged (orcaMonitor);
         jqUnit.assertEquals (
