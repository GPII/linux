  ...
  var path = require ("path"),
    ...
    jqUnit.test (
      ...
      function() {
        ...
        // process.
        var nodeProc = processes.findProcessByPid (process.pid, procInfos);
    ...
    jqUnit.test (
      ...
      function() {
        jqUnit.assertNull (
          "Search negative process id value", processes.findProcessByPid (-1)
        );
    });
    ...
    jqUnit.test (
        ...
        function() {
          var nodeProcInfo = processes.findProcessByPid (process.pid);
          ...
          // returns only the basename for argv[1], whereas node has the full
          // path.  Compare using Path.basename().
          for (var i = 0; i < process.argv.length; i++) {
            var processArg = process.argv[i];
            if (i === 1) {
              processArg = path.basename (processArg);
            }
            jqUnit.assertEquals ("Node process 'argv[" + i + "]'",
                                 processArg, nodeProcInfo.argv[i]);
          }
    });
    ...
    jqUnit.test (
      ...
      function() {
        var nodeProcInfos = processes.findProcessesByCommand ("node");
        jqUnit.assertNotEquals (
        ...
        var procInfo = processes.findFirstProcessByCommand ("node");
    ...
    jqUnit.test (
      ...
      function() {
        var notRunning = processes.initProcInfoNotRunning ("grep");
        ...
        jqUnit.assertNull (
          ...
          processes.findProcessByPid (notRunning.pid)
        );
    });
    ...
    jqUnit.test (
      ...
      function() {
        var procInfo = processes.findProcessByPid (process.pid);
        jqUnit.assertNotNull ("Searching for 'node' process", procInfo);
        jqUnit.assertTrue (
          "Check nodejs is running", processes.isRunning (procInfo.state)
        );
        procInfo = processes.initProcInfoNotRunning ("grep");
        jqUnit.assertFalse (
          "Check nonexistent process running", processes.isRunning (procInfo)
        );
    });
    ...
    jqUnit.test (
      ...
      function() {
        ...
        var newProcInfo = processes.updateProcInfo (procInfo);
        jqUnit.assertDeepEq (
    ...
    jqUnit.test (
      ...
      function() {
        ...
        var grepInfo = processes.findProcessByPid (grep.pid);
        jqUnit.assertNotNull ("Search 'grep' process", grepInfo);
        jqUnit.assertTrue ("Stop grep", grep.kill ("SIGHUP"));
        var newGrepInfo = processes.updateProcInfo (grepInfo);
        jqUnit.assertNotEquals (
    ...
    jqUnit.test (
      ...
      function() {
        jqUnit.assertFalse (
          "Check null monitor", processes.hasStateChanged (null)
        );
        var catMonitor = processes.initMonitor (null);
        jqUnit.assertFalse (
          "Check null process", processes.hasStateChanged (catMonitor)
        );
        var catProcInfo = processes.initProcInfoNotRunning ("cat");
        catMonitor = processes.initMonitor (catProcInfo);
        var stateChanged = processes.hasStateChanged (catMonitor);
        jqUnit.assertFalse ("Check non-running process", stateChanged);

        var cat = spawn ("cat");
        catMonitor = processes.initMonitor (catProcInfo);
        stateChanged = processes.hasStateChanged (catMonitor);
        jqUnit.assertTrue ("Check running process", stateChanged);

        // Get the running process info, kill cat, and check again.
        catProcInfo = processes.findProcessByPid (cat.pid);
        catMonitor = processes.initMonitor (catProcInfo);
        cat.kill ("SIGHUP");
        stateChanged = processes.hasStateChanged (catMonitor);
    ...
    jqUnit.test (
      ...
      function() {
        jqUnit.assertFalse (
          "Check null monitor", processes.hasSwitchedRunState (null)
        );
        var grepProcMonitor = processes.initMonitor (null);
        jqUnit.assertFalse (
          "Check null process", processes.hasSwitchedRunState (grepProcMonitor)
        );
        var grep = spawn ("grep", ["ssh"]);
        var grepProcInfo = processes.findProcessByPid (grep.pid);
        grepProcMonitor = processes.initMonitor (grepProcInfo);
        var switched = processes.hasSwitchedRunState (grepProcMonitor);
        ...
        grep.kill ("SIGHUP");
        switched = processes.hasSwitchedRunState (grepProcMonitor);
    ...
    jqUnit.test (
      "Test findSolutionsByCommands()",
        function() {
          ...
          var procInfos = processes.findSolutionsByCommands (solutions);
    ...
    jqUnit.test (
      "Test findSolutionsByPids()",
        function() {
          ...
          var procInfos = processes.findSolutionsByPids (pids);
    ...
    jqUnit.test (
      "Test gpii.processes against launching orca.",
      function() {
        var orcaProcInfo = processes.findFirstProcessByCommand ("orca");
        ...
        procTests.wait5sec();
        orcaProcInfo = processes.findFirstProcessByCommand ("orca");
        jqUnit.assertNotNull ("Orca is running", orcaProcInfo);
        ...
        procTests.wait5sec();
        var orcaProcNewState = processes.findProcessByPid (orcaProcInfo.pid);
        jqUnit.assertNull ("Orca no longer running", orcaProcNewState);
        var orcaMonitor = processes.initMonitor (orcaProcInfo);
        processes.hasStateChanged (orcaMonitor);
        jqUnit.assertEquals (
