...
(function() {
    ...
    function (commandName, procArray) {
      var commands = gpii.processes.findProcessesByCommand (commandName, procArray);
    ...
    // Return an object { switched, newProcInfo }.
    // - if the state has not changed, switch = false.
    // - if the state has changed, switch = true.  Special case:  a
    //   process that has quit isn't in the processes list; return
    //   "NoSuchProcess".
    gpii.processes.hasSwitchedRunState = function (monitor) {
      if (!monitor.procInfo) {
        // nothing sought === nothing changed.
//        return { runStateChanged: false, newProcInfo: null };
        return false;
      }
      var newInfo = null;
      if (monitor.procInfo.state === "NoSuchProcess") {
        newInfo = gpii.processes.findFirstProcessByCommand (monitor.procInfo.command);
      }
      else {
        newInfo = gpii.processes.findProcessByPid (monitor.procInfo.pid);
      }
      var wasRunning = gpii.processes.isRunning (monitor.procInfo.state);
      var isRunning = ( newInfo !== null ?
                        gpii.processes.isRunning (newInfo.state) :
                        false );
      monitor.newProcInfo = newInfo;
      return (isRunning !== wasRunning);
    };

    // Renew the information on a process, or create a new "NoSuchProcces".
    gpii.processes.refreshProcInfo = function (procInfo) {
      ...
      if (newProcInfo === null) {
        newProcInfo = gpii.processes.initNotRunning (procInfo.command);
      }
      return newProcInfo;
    };
    ...
    // Create a monitor object for passing to a functiona that periodically
    ...
    gpii.processes.monitorRunningChanged = function (monitor) {

      // Check for switch in running state.  If changed, refresh the processs
      // information.
      if (gpii.processes.hasSwitchedRunState (monitor)) {
        var oldState = monitor.procInfo.state;
        if (monitor.newProcInfo === null) {
          monitor.procInfo = gpii.processes.initNotRunning (monitor.procInfo.command);
        }
        else {
        ...
        console.log ("SWITCHED from " + oldState + " to " + monitor.newState);
        console.log ("");
      }
    };
}());