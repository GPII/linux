...
(function() {
    ...
    processes.getProcesses = function() {
    ...
    processes.findProcessByPid = function (pid, procArray) {
      if (!procArray) {
    ...
    processes.findProcessesByCommand = function (commandName, procArray) {
      if (!procArray) {
        procArray = processes.getProcesses();
      }
    ...
    //   "NoSuchProcess".
    processes.hasStateChanged = function (procInfo) {
      if (!procInfo) {
      ...
      var theProc = processes.findProcessByPid (procInfo.pid, procArray);

      // Case:  process not in the current list, implies state changed to being
      // quit or killed.
      if (theProc === null) {
    ...
    processes.refreshProcInfo = function (pid) {
      var procArray = processes.getProcesses();
      return processes.findProcessByPid (pid, procArray);
    };

    // Create a monitor object for passing to a setInterval() to periodically
    // check for chanages in the status of a process.
    processes.initMonitor = function() {
      var monitor = {};
    ...
    // process.
    processes.monitorChanged = function (procInfo, monitor) {
      var newState = processes.hasStateChanged (procInfo);
      console.log ("processes.monitorChanged():  <newState> is " + newState);
      if (newState) {
