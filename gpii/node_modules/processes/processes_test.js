...
(function() {
  ...
  var path = require ("path"),
    ...
    jqUnit.test (
      ...
      function() {
        jqUnit.assertFalse (
          "Check null process", processes.hasStateChanged (null)
        );
        var catProcInfo = processes.initProcInfoNotRunning ("cat");
        var stateChanged = processes.hasStateChanged (catProcInfo);
        jqUnit.assertFalse ("Check non-running process", stateChanged);

        var cat = spawn ("cat");
        stateChanged = processes.hasStateChanged (catProcInfo);
        jqUnit.assertTrue ("Check running process", stateChanged !== false);

        // Get the running process info, kill cat, and check again.
        catProcInfo = processes.findProcessByPid (cat.pid);
        cat.kill ("SIGHUP");
        stateChanged = processes.hasStateChanged (catProcInfo);
        jqUnit.assertTrue ("Check stopped process", stateChanged !== false);
    });
    ...
    jqUnit.test (
      ...
      function() {
        ...
        var newState = processes.hasStateChanged (orcaProcInfo);
        jqUnit.assertEquals (
          "Orca process changed state", "NoSuchProcess", newState
        );
        // Clean up.
        if (wasRunning) {
          nodeGSettings.set_gsetting (
