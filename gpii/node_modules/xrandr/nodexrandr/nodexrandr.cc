...
NAN_METHOD(getDisplays) {
    ...
    Local<Object> resolution_object = New<Object>();
    ...
    for (int i=0; i<res->noutput; i++) {
        ...
        output->Set(New("name").ToLocalChecked(),
                    New(output_info->name).ToLocalChecked());
        switch (output_info->connection) {
            case RR_Connected:
                output->Set(New("status").ToLocalChecked(),
                            New("connected").ToLocalChecked());

                resolution_object->Set(New("width").ToLocalChecked(),
                                       New(sizes[current_size].width));
                resolution_object->Set(New("height").ToLocalChecked(),
                                       New(sizes[current_size].height));

                resolution_object->Set(New("mwidth").ToLocalChecked(),
                                       New(sizes[current_size].mwidth));
                resolution_object->Set(New("mheight").ToLocalChecked(),
                                       New(sizes[current_size].mheight));

                output->Set(New("resolution").ToLocalChecked(), resolution_object);
                break;
            case RR_Disconnected:
                output->Set(New("status").ToLocalChecked(),
                            New("disconnected").ToLocalChecked());
                break;
        }

        Local<Array> available_resolutions = New<Array>();
        ...
        output->Set(New("available_resolutions").ToLocalChecked(),
...
NAN_METHOD(setScreenResolution) {
    ...
    if (dpy == NULL) {
        ...
        return info.GetReturnValue().Set(New(False));
    }
    ...
    XRRFreeScreenConfigInfo (sc);
    info.GetReturnValue().Set(New(True));
}
