...
(function () {
    ...
    test("gpii.linux.launchHandlers.gsettingsLaunchHandler.payloadHasErrors() function", function () {
        expect(2);
        //test set:
        var retVal = gpii.linux.launchHandlers.gsettingsLaunchHandler.payloadHasErrors(errorPayload);
        deepEqual(retVal, { statusCode: 500, data: errorPayload}, "Testing payload with error");

        retVal = gpii.linux.launchHandlers.gsettingsLaunchHandler.payloadHasErrors(noErrorPayload);
        equal(retVal, false, "Testing payload without error");
    });

    asyncTest("gpii.linux.launchHandlers.gsettingsLaunchHandler.run() function", function () {
        expect(4);
        var retVal = gpii.linux.launchHandlers.gsettingsLaunchHandler.run(magnifierPayload.start);
        equal(retVal, { statusCode: 200 }, "Launching with valid payload should give success statusCode");
        ...
        child_process.spawn("gsettings get org.gnome.desktop.a11y.applications screen-magnifier-enabled", function(error, stdout, stderr) {
            ...
            equal(stdout, true, "Expect the magnifier to be set");
        });
        ...
        child_process.spawn("gsettings get org.gnome.desktop.a11y.applications screen-magnifier-enabled", function(error, stdout, stderr) {
            console.log("stdout: "+stdout);
            equal(stdout, false, "Expect the magnifier to be unset");
        });
    });
}());