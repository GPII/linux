...
makePac (PkPackage *package) {
  ...
  pac->Set(String::New("arch"),
           String::New (pk_package_get_arch(package)));
 ...
 */
Handle<Value> performAction(const Arguments& args) {
  HandleScope scope;
  PkTask *task;
  GError *err = NULL;
  std::basic_string<gchar> action = utf8StringFromValue(args[0]);
  std::basic_string<gchar> name = utf8StringFromValue(args[1]);
...
void init(Handle<Object> target) {
  ...
  target->Set(String::NewSymbol("performAction"),
              FunctionTemplate::New(performAction)->GetFunction());
}
