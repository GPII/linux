...
(function() {
  ...
  pkTests.initTuxguitar = function() {
      ...
      pkTests.tuxguitar.pkgList = pkgs;
      pkTests.tuxguitar.pkg = pkTests.findPackageByName ("tuxguitar", pkgs);
      return pkTests.tuxguitar;
  };

  pkTests.isTuxguitarInList = function (pkgList) {
      var tuxguitar = pkTests.tuxguitar;
  ...
  pkTests.initSearchTest = function() {
      ...
      tuxguitar.inInstalledList = pkTests.isTuxguitarInList (installedPkgs);
      var availablePkgs = packagekit.searchPackage ("tuxguitar", "~installed");
      tuxguitar.inAvailableList = pkTests.isTuxguitarInList (availablePkgs);
      return tuxguitar;
  };
  ...
  pkTests.runInstalledVsAvailableTests = function (msg) {
      var tuxguitar = pkTests.tuxguitar;
  ...
  pkTests.searchForMatch = function (searchFilter) {
      var tuxguitar = pkTests.tuxguitar;
  ...
  jqUnit.test ("Test searchPackage() with 'tuxguitar' comparing installed " +
      ...
      pkTests.initSearchTest();
      pkTests.runInstalledVsAvailableTests ("Search");
  });

  jqUnit.test ("Test getPackages() with tuxguitar comparing installed vs. " +
               "available filters.", function() {
      pkTests.initGetTest();
      pkTests.runInstalledVsAvailableTests ("Get");
  });
 ...
 jqUnit.test ("Test performAction(): 'install' and 'remove' tuxguitar." +
      ...
      var matchPkg = null;
      if (pkTests.isInstalled (tuxguitar.pkg)) {
          packagekit.performAction ("remove", tuxguitar.pkg.id);
          matchPkg = pkTests.searchForMatch ("~installed");
      ...
      // ...if it isn't installed, test installing it and then removing it.
          ...
          packagekit.performAction ("install", tuxguitar.pkg.id);
          matchPkg = pkTests.searchForMatch ("installed");
